...
public class NaverLoginBO {
	...
	// 어떤 아이디로 소셜로그인을 하는지 감지하기위해서 인증 URL생성(로그인 할 수 있는 경로)
	public String getAuthorizationUrl(HttpSession session) {
		// 세션 유효성을 검증하기위해 난수를 생성
		String state = generateRandomString();
		// 생성한 난수 값을 session에 저장
		setSession(session, state);
		// Scribe에서 제공하는 인증 URL 생성 기능을 이용해 네아로 인증 URL 생성
		OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET)
				.callback(REDIRECT_URI).state(state)	// 앞서 생성한 난수값을 인증 URL생성시 사용
				.build(NaverLoginApi.instance());
		
		return oauthService.getAuthorizationUrl();
	}
	
	// 네아로 Callback 처리 및 접근이 성공했을 경우 AccessToken 획득 메서드
	...
	// Access Token을 이용해 네이버 사용자 정보에 대한 API를 호출
